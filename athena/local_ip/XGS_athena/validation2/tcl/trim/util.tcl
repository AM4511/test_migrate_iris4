...
proc a {} {
    ...
    foreach COLOR_SPACE $COLOR_SPACE_RANGE {
	foreach X_SIZE $X_SIZE_RANGE {
		...
		foreach X_REVERSE $X_REVERSE_RANGE {
			...
			# set PARAMETER_LIST "{-GTEST_NAME=${test_name}} -gCOLOR_SPACE=${COLOR_SPACE} -gY_SIZE=${Y_SIZE} -gX_SIZE=${X_SIZE} -gX_ROI_EN=${X_ROI_EN} -gX_ROI_START=${X_ROI_START} -gX_ROI_SIZE=${X_ROI_SIZE} -gX_REVERSE=${X_REVERSE} {-GX_SCALING=${X_SCALING}}"
			set logfile "${test_name}.log"

			vsim -gui work.testbench -do "${IP}/validation2/tcl/valid.do" -GTEST_NAME=${test_name} -GCOLOR_SPACE=${COLOR_SPACE} -GY_SIZE=${Y_SIZE} -GX_SIZE=${X_SIZE} -GX_ROI_EN=${X_ROI_EN} -GX_ROI_START=${X_ROI_START} -GX_ROI_SIZE=${X_ROI_SIZE} -GX_REVERSE=${X_REVERSE} -GX_SCALING=${X_SCALING} -GY_ROI_EN=${Y_ROI_EN} -GY_ROI_SIZE=${Y_ROI_SIZE} -GY_ROI_START=${Y_ROI_START} -donotcollapsepartiallydriven -permit_unmatched_virtual_intf -l ${logfile}
			run -all
