...
#
# Example      : source $env(IRIS4)/athena/backend/zynq_dev/create_athena_zc706.tcl
#
# ##################################################################################
...
set BASE_NAME athena_zc706_dev
...
set BACKEND_DIR ${WORKDIR}/backend/zynq_dev
...
if {${QUICK_FLOW} > 0} {
	puts "Enabling QUICK Flow mode..."
	set_property AUTO_INCREMENTAL_CHECKPOINT 1  [get_runs ${SYNTH_RUN}]
	set_property strategy Flow_RuntimeOptimized [get_runs ${SYNTH_RUN}]
	set_property AUTO_INCREMENTAL_CHECKPOINT 1  [get_runs ${IMPL_RUN}]
	set_property strategy Flow_Quick            [get_runs ${IMPL_RUN}]
}
