...
################################################
set FILE_LIST [list \
  [file normalize "${SRC_DIR}/ares_test.vhd"]
]
...
add_files -fileset ${CONSTRAINTS_FILESET} -norecurse ${SYSTEM_DIR}/compile.xdc
add_files -fileset ${CONSTRAINTS_FILESET} -norecurse ${SYSTEM_DIR}/hyperbus133MHz.sdc
add_files -fileset ${CONSTRAINTS_FILESET} -norecurse ${SYSTEM_DIR}/timing_late.sdc
set_property PROCESSING_ORDER LATE [get_files  ${SYSTEM_DIR}/timing_late.sdc]

set_property used_in_synthesis false [get_files  ${SYSTEM_DIR}/compile.xdc]
# Needs to be processed late because of the set_property IOB false constraints
set_property PROCESSING_ORDER LATE   [get_files  ${SYSTEM_DIR}/compile.xdc]
